...
export default function QRPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const walletResponse = await apiClient.get(`/wallet/${userId}`);
        setWalletBalance(walletResponse.data.balance);

        const budgetResponse = await apiClient.get(`/budget/${userId}`);
        setCategories(
  ...
  const handleFinalPay = async () => {
    ...
    try {
      ...
      const payload = {
        senderId: userId,
        recipientEmail: upiId,
        amount: paymentAmount,
        description: description || "Payment from wallet",
      };

      await apiClient.post("/wallet/pay-to-user", payload);
