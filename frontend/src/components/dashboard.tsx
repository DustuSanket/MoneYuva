...
const useBudgetState = () => {
  ...
  useEffect(() => {
    setIsClient(true);
    // Your API call to fetch the budget goes here
  }, []);
    ...
    async (newBudget: number) => {
      ...
      if (newBudget > 0) {
        try {
          await axios.post(
            `${API_BASE_URL}/budget`,
            { userId, monthlyBudget: newBudget, categories: [] },
            {
          ...
          toast({
            title: "Budget Set!",
            description: "Your monthly budget is ready.",
          });
        } catch (error) {
          ...
          toast({
            variant: "destructive",
            title: "Error",
            description: "Could not set budget.",
          });
        }
      } else {
        toast({
...
const useCategoriesState = (budget: number | null) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        setCategories(
          budgetResponse.data.categories.map((c: any) => ({
            ...c,
  ...
  const unallocatedBudget = useMemo(
    () => (budget || 0) - totalAllocated,
    [budget, totalAllocated]
  );

  const addCategory = useCallback(
    async (name: string, allocated: number, iconName: string) => {
      const token = localStorage.getItem("token");
      const userId = localStorage.getItem("userId");
      if (!userId || !token) return;
        ...
        toast({
          ...
          description: (
            <>
              ...
              <IndianRupee className="w-3.5 h-3.5 inline-flex" />
      ...
      try {
        const updatedCategories = [
          ...categories,
          { id: Date.now().toString(), name, allocated, spent: 0 },
        ];
        await axios.put(
          `${API_BASE_URL}/budget/${userId}`,
        ...
        toast({
          ...
          description: `${name} category created.`,
        });
      } catch (error) {
    ...
    [unallocatedBudget, categories, toast]
  );
  ...
  const addExpense = useCallback(
        ...
        toast({
          ...
          description: (
            <div className="flex items-center">
              ...
              {amount.toFixed(2)} spent from {category.name}.
            </div>
          ),
        });
      } catch (error) {
    ...
    [walletBalance, categories, toast]
  );
...
const BudgetSetup = React.memo(
  ({ onSetBudget }: { onSetBudget: (budget: number) => void }) => {
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
      e.preventDefault();
      const newBudget = parseFloat(
        (e.currentTarget.elements.namedItem("budget") as HTMLInputElement).value
      );
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="budget" className="flex items-center">
                  Total Monthly Budget (<IndianRupee className="w-4 h-4 mx-1" />
                  )
                </Label>
                <Input
                  id="budget"
...
const BudgetSummary = React.memo(
  ({
    budget,
    walletBalance,
    categories,
    ...
    return (
      <>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-grow">
                ...
                <p className="text-sm text-muted-foreground flex items-center">
                  ...
                  {category.allocated.toFixed(2)} spent
                </p>
        ...
        >
          <AddExpenseDialog
            category={category}
            onAddExpense={(amount) => onAddExpense(category.id, amount)}
    ...
    <div className="space-y-6">
      ...
      {unallocatedBudget > 0 && (
        <div className="text-center text-muted-foreground text-sm flex items-center justify-center">
          You have{" "}
          <span className="font-bold text-primary flex items-center mx-1">
        ...
        <Card className="text-center py-12">
          <CardContent>
            ...
            <p className="text-muted-foreground mt-2">
              Click "Add Category" to start allocating your budget.
            </p>
          </CardContent>
        </Card>
      ) : (
...
const iconOptions = [
  { name: "Pocket Money", icon: Package },
  { name: "Home", icon: Home },
...
const AddCategoryDialog: React.FC<{
  onAddCategory: (name: string, allocated: number, iconName: string) => void;
  unallocatedBudget: number;
}> = ({ onAddCategory, unallocatedBudget }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    await onAddCategory(name, parseFloat(allocated), selectedIcon);
    setLoading(false);
    setIsOpen(false);
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              min="0.01"
              step="0.01"
              max={unallocatedBudget}
            />
            <p className="text-xs text-muted-foreground flex items-center">
              Unallocated budget: <IndianRupee className="w-3 h-3 mx-0.5" />
              {unallocatedBudget.toFixed(2)}
            </p>
          </div>
          <DialogFooter>
...
const AddExpenseDialog: React.FC<{
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (parseFloat(amount) > 0) {
      await onAddExpense(category.id, parseFloat(amount));
    }
    setLoading(false);
    onClose();
  };
    ...
    <DialogContent>
      <DialogHeader>
        ...
        <CardDescription className="flex items-center">
          Remaining: <IndianRupee className="w-4 h-4 mx-1" />
          {(category.allocated - category.spent).toFixed(2)}
        </CardDescription>
      </DialogHeader>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="space-y-2">
          <Label htmlFor="exp-amount" className="flex items-center">
            Expense Amount (<IndianRupee className="w-3.5 h-3.5 mx-1" />)
          </Label>
          <Input
            id="exp-amount"
            value={amount}
            ...
            min="0.01"
            step="0.01"
            max={category.allocated - category.spent}
            autoFocus
          />
        </div>
        <DialogFooter>
